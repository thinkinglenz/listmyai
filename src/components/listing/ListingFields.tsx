...
// Every editable field of a listing, shared by the owner's edit page and the
// admin's full edit page so the two can never drift apart. The owner form was
// the only place that had all of them; admin had eight.

import { useState } from 'react'
import PreviewImageUpload from '@/components/PreviewImageUpload'
import MediaUpload from '@/components/MediaUpload'
import { Globe, Video, DollarSign, Building2, Target, Megaphone, ChevronDown, ChevronUp } from 'lucide-react'
...
export { Field, Section, Toggle, inputCls, inputStyle, selectStyle, CATEGORIES, PRICING_MODELS, PRICING_LABELS }

export default function ListingFields({ tool, set, children }: {
  tool: Tool
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  set: (key: string, value: any) => void
  /** Admin-only controls (status, category, flags), rendered above the rest. */
